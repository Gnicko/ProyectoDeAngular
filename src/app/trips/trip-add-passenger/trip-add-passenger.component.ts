...
export class TripAddPassengerComponent {
  ...
  save(selection: SelectionModel<Person>) {

    if (this.trip) {
      if (!selection.isEmpty()) {
        this.personsArray = selection.selected;

        if (this.personsArray.length <= this.trip?.validateFreeSpace()) {
        ...
        this.snackBar.open("No se selecciono ninguna persona", "Error", {duration: 2000});
      }
    }
  }
